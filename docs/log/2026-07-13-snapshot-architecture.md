...
Reports compare complete publication editions rather than treating every row seen anywhere in an ISO week as current. A series present in the prior edition but absent from the latest is labeled “not reported,” not “removed.” A series missing from the prior edition but seen in an older edition is labeled “returned after absence”; only a series with no older observation is “newly reported.” Charts continue to aggregate by ISO week and retain historical series with gaps instead of deleting their history when a later edition omits them.
